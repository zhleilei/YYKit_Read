...
/// Whether this app is pirated (not install from appstore). 盗版
@property (nonatomic, readonly) BOOL isPirated;

/// Whether this app is being debugged (debugger attached). 是否正在调试此应用程序（附加调试器）。
@property (nonatomic, readonly) BOOL isBeingDebugged;

/// Current thread real memory used in byte. (-1 when error occurs) 当前线程实内存以字节为单位
@property (nonatomic, readonly) int64_t memoryUsage;

/// Current thread CPU usage, 1.0 means 100%. (-1 when error occurs) 当前线程CPU使用率，1.0表示100％。（发生错误时为-1）
...
/**
 ...
 增加活动网络请求的数量。如果在递增之前该数字为零，它将显示活动指示器。
...
/**
 ...
 status bar network activity indicator.
 减少活动网络请求的数量。如果此数字在递减后变为零，则将停止显示活动指示器。

...
/**
 If you have 3 pair of points transformed by a same CGAffineTransform:
     ...
     p2 (transform->) q2
     p3 (transform->) q3
 This method returns the original transform matrix from these 3 pair of points. 此方法从这三对点返回原始变换矩阵。
...
/// Get the transform which can converts a point from the coordinate system of a given view to another. 获取可以将一个点从给定视图的坐标系转换到另一个视图的变换。
CGAffineTransform YYCGAffineTransformGetFromViews(UIView *from, UIView *to);

/// Create a skew transform. 创建偏斜变换
...
/// Negates/inverts a UIEdgeInsets. 取反/反转UIEdgeInsets。
static inline UIEdgeInsets UIEdgeInsetsInvert(UIEdgeInsets insets) {
    return UIEdgeInsetsMake(-insets.top, -insets.left, -insets.bottom, -insets.right);
}

/// Convert CALayer's gravity(重力) string to UIViewContentMode.  ???
...
/// Returns the center for the rectangle. 返回矩形的中心点
static inline CGPoint CGRectGetCenter(CGRect rect) {
    return CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
}

/// Returns the area of the rectangle. 返回矩形面积
...
/// Returns the distance between two points. 两点之间的距离
static inline CGFloat CGPointGetDistanceToPoint(CGPoint p1, CGPoint p2) {
    return sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}

/// Returns the minmium distance between a point to a rectangle. 返回点到矩形之间的最小距离。
...
/// floor point value for pixel-aligned 像素对齐的底点值
static inline CGFloat CGFloatPixelFloor(CGFloat value) {
    CGFloat scale = YYScreenScale();
    return floor(value * scale) / scale;
}

/// round point value for pixel-aligned 像素对齐的圆点值 ??
...
/// round point value to .5 pixel for path stroke (odd pixel line width pixel-aligned) 路径笔划圆点值为.5像素（单像素线宽，像素对齐）

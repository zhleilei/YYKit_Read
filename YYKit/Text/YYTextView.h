...
/**
 ...
 YYTextViewDelegate协议定义了一组可以使用的可选方法

 接收YYTextView对象的与编辑有关的消息。
...
/**
 ...
 容器中的文本垂直对齐方式。
 默认值为YYText VerticalAlignment Top。
 */
...
/**
 ...
 在正常状态下应用于链接的属性。 默认为浅蓝色。 当dataDetectorTypes检测到一定范围的文本时，该值为用于修改范围内的原始属性。
 */
...
/**
 ...
 当文本视图成为第一响应者时显示的定制输入视图。它可以用来代替系统键盘。
...
/**
 ...
 如果您使用不带“ inputAccessoryView”属性的自定义附件视图，
 您可以设置附件视图的高度。
 自动滚动计算可能会使用它。
 */

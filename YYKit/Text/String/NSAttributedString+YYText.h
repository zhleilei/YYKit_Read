...
/**
 The skew(斜) to be applied(应用) to glyphs(字形). (read-only)
 应用于字形的偏斜。（只读）
 ...
 @since UIKit:7.0
 */
@property (nullable, nonatomic, strong, readonly) NSNumber *obliqueness; // 斜度
...
/**
 Specifies(指定) a bidirectional(双向的) override(覆写) or embedding(嵌入). (read-only)
 指定双向替代或嵌入。
 （只读）
...
/**
 ...
 NSParagraphStyle对象，用于指定类似线对齐，制表尺，书写方向等（只读）
...
/**
 ...
 @since CoreText:6.0  UIKit:6.0  YYKit:6.0
 */
@property (nonatomic, readonly) CGFloat lineSpacing;//  行间距
...
/**
 The paragraph's threshold(阈) for hyphenation(连字). (A wrapper for NSParagraphStyle). (read-only)
 该段落的断字阈值。（NSParagraphStyle的包装）。（只读）
 ...
 @since UIKit:6.0
 */
@property (nonatomic, readonly) float hyphenationFactor; // 连字因子
...
/**
 ...
 文档范围内的默认制表符间隔（NSParagraphStyle的包装器）。
 （只读）
...
/**
 ...
 @since CoreText:7.0  UIKit:7.0  YYKit:7.0
 */
#warning zll tabStops
@property (nullable, nonatomic, copy, readonly) NSArray<NSTextTab *> *tabStops; // 制表位
...
///=============================================================================
/// @name Create attachment string for YYText 为YYText创建附件字符串
///=============================================================================
...
/**
 ...
 @param descent      The attachment's container descent in layout. 附件的容器布局下降。
...
/**
 Creates and returns an attahment from a fourquare(四方) image as if it was an emoji.

...
/// Use this attribute to bind a range of text together, as if it was a single charactor. 使用此属性可以将一系列文本绑定在一起，就好像它是单个字符一样。
...
/// 阴影将在文字字形下方绘制。使用YYTextShadow.subShadow添加多阴影。
UIKIT_EXTERN NSString *const YYTextShadowAttributeName;

/// The value of this attribute is a `YYTextShadow` object.
/// Use this attribute to add inner shadow to a range of text.
/// Inner shadow will be drawn above text glyphs. Use YYTextShadow.subShadow to add multi-shadow.内部阴影将绘制在文字字形上方。使用YYTextShadow.subShadow添加多阴影。
...
/// 使用此属性可将删除线（删除行）添加到文本范围。
/// The strikethrough will be drawn above text glyphs. 删除线将绘制在文字字形上方。
...
/// 该属性的值是一个NSValue对象，用于存储CGAffineTransform。
...
@property (nullable, nonatomic, copy) NSString *string; ///< backed string 支持的字符串
@end


/**
 YYTextBinding objects are used by the NSAttributedString class cluster(簇)
...
/**
 ...
 as the values for decoration(装饰) line attributes (stored in the attributed string under
